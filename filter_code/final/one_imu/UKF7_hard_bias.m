function [x_hatM, P_hatM] = UKF7_hard_bias(data,t,Q,R)
...
z(1:3,:) = z(1:3,:) - mean_a; %measurements
z(3,:) = z(3,:) + 9.81;
z(4:6,:) = z(4:6,:) - mean_w;
%% states
% 1: G_R_I = 4x1 quat
...
for itr = 1: n_steps-1
    dt = t(itr+1) - t(itr);
    %PREDICTION
    X = gen_sigma_points7(x_hatM(:,itr),P_hatM(:,:,itr) + Q); 
%     figure(1)
%     surf(X); xlabel('x');ylabel('y');title('X');
    for i_sp=1:n_sigma_points
        Y(1:4,i_sp) = quatproduct(X(:,i_sp), aa2quat(z(4:6,itr)*dt));
        Y(5:7,i_sp) = X(5:7,i_sp);
    end 
%     figure(2)
%     surf(Y); xlabel('x');ylabel('y');title('Y');
    [x_ap, P_ap, W_prime] = Y_stats7(Y,alpha_mu,alpha_cov,x_hatM(:,itr));% stats from Y. W_prime: Y with x_ap subtracted from each. W_prime in vector space
    
    %UPDATE  
    for i_sp = 1:n_sigma_points
        tempg = quatproduct(g_quat,Y(1:4,i_sp));
        temp = quatproduct([Y(1,i_sp);-Y(2:4,i_sp)],tempg);
        Z(:,i_sp) = [temp(2:4); Y(4:6,i_sp)];
    end 
    
    [z_ap, P_zz] = Z_stats7(Z,alpha_mu,alpha_cov);
    ...
    x_hat = [quatproduct(x_ap(1:4), aa2quat(Kv(1:3))); x_ap(5:7)+Kv(4:6)];
    P_hat = P_ap - K*P_nu'*K';

function [x_hatM, P_hatM] = UKF7bias(data,t,Q,R)
...
%     surf(Y); xlabel('x');ylabel('y');title('Y');
    ...
    for i_sp = 1:n_sigma_points
        ...
        temp_accel = quatproduct([Y(1,i_sp);-Y(2:4,i_sp)],tempg);

        bias = Y(8:10,i_sp);
        temp_gyro = Y(4:6,i_sp) - bias;

        Z(:,i_sp) = [temp_accel(2:4); temp_gyro];
    end 
    ...
    x_hat = [quatproduct(x_ap(1:4), aa2quat(Kv(1:3))); x_ap(5:end)+Kv(4:end)];
    P_hat = P_ap - K*P_nu'*K';

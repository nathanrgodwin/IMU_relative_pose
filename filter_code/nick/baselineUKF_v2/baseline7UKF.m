function [x_hatM,fig] = baseline7UKF(data,t)
...
%     surf(Y); xlabel('x');ylabel('y');title('Y');
    [x_ap, P_ap, W_prime] = Y7_stats(Y,alpha_mu,alpha_cov,x_hatM(:,itr));% stats from Y. W_prime: Y with x_ap subtracted from each. W_prime in vector space
    ...
    [z_ap, P_zz] = Z7_stats(Z,alpha_mu,alpha_cov, z_apM(:,itr));
